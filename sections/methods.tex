...
% ✅ CLEARED August 12, 2026. This paragraph was marked FALSE-as-built: the
% numerator summed every NAICS hierarchy level while the denominator summed one,
% so the measure was not the share described here. The rebuild delivers the
% intended measure -- sector-level rows only, shares bounded by one, median 0.206
% against a national figure near 0.19 -- so the paragraph is now true as written.
% The 2,889 and 2,887 counts it once carried are gone; see the count block below,
% which is read off model output rather than derived.
% Rebuild: causal-did/Dalis_Abdallah_RebuildExposure.R.
% Diagnosis: causal-did/EXPOSURE_DEFECT_2026-08-12.md.
...
% ⚠️ CITATION ADDED August 12, 2026. The method was used for four sections
% without being attributed. `jorda2005` was already carried in the key list and
% was the only entry in references.bib with no \cite anywhere, which is the
% shape of an omission rather than a decision.
% SOURCE: monetary_policy_labor.R lines 176-203.
The second design follows the local-projection approach of \citet{jorda2005}. At
each horizon $h=0,\dots,12$ it estimates
%
\begin{equation}
...
% ⚠️ SCALE WARNING. A reader will otherwise try to compare the two coefficients
% directly. On the rebuilt measure the numbers are 0.033 (difference-in-
% differences) against 0.158 (local projection, h = 0); they were 0.022 and 0.198
% before the rebuild. The warning is about the scaling, not about those specific
% values, and holds either way.
